...
class ResemblanceExample(Example):
    segment_1: Text
    segment_2: Text
    label: Optional[Resemblance]
    index: Optional[int]
    original_id: int
...
class ResemblanceCorpus(Corpus):
    name = 'Resemblance'
    ...
    def get_num_of_labels(self) -> int:
        num = len(Resemblance.__members__)
        return num
    ...
    def _get_examples(self, ds_type: DataSetType, *args, **kwargs):
        ...
        for (i, line) in enumerate(lines):
            ...
            if label == 'elab':
                label = Resemblance.elab
            elif label == 'par':
                label = Resemblance.par
            elif label == 'contr':
                label = Resemblance.contr
            else:
            ...
            e = ResemblanceExample(id=guid, type=ds_type, segment_1=segment_obj_1, segment_2=segment_obj_2,
