...
from data.corpus._utils import ItemsForMetricsComputation, acc_and_f1_muti
...
class CoherenceExample(Example):
    segment_1: Text
    segment_2: Text
    label: Optional[Coherence]
    index: Optional[int]
    original_id: int
...
class CoherenceCorpus(Corpus):
    ...
    label_type = Coherence
    data_path = file_tool.dirname(__file__)
    raw_filename_dict = {
        DataSetType.train: file_tool.connect_path(data_path, 'raw', 'train.txt'),
        DataSetType.dev: file_tool.connect_path(data_path, 'raw', 'dev.txt'),
        DataSetType.test: file_tool.connect_path(data_path, 'raw', 'test.txt')
    }
    ...
    def _get_examples(self, ds_type: DataSetType, *args, **kwargs):
        ...
        for (i, line) in enumerate(lines):
            ...
            if label == 'temp':
                label = Coherence.temp
            elif label == 'attr':
                label = Coherence.attr
            elif label == 'ce':
                label = Coherence.ce
            elif label == 'resemblance':
                label = Coherence.resemblance
            else:
            ...
            e = CoherenceExample(id=guid, type=ds_type, segment_1=segment_obj_1, segment_2=segment_obj_2,
    ...
    def compute_metrics(self, itmes: ItemsForMetricsComputation):
        return acc_and_f1_muti(itmes.predictions, itmes.label_ids)
